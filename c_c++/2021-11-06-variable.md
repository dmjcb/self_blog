...
`extern` 告知编译器该变量在其他文件中定义, 当前文件只引用, 不分配内存

### 定义

定义会为变量分配内存, 并可为其赋初值, 变量只能定义一次
...
局部变量在栈上分配, 作用域仅在函数内部, 生命周期与函数调用周期相同
...
局部静态变量在静态存储区分配, 作用域仅在函数内部, 但生命周期与程序相同可在多次函数调用间保持状态
...
局部static变量在函数内部定义, 能在多次调用之间保持其值, 它只在第一次调用时初始化, 之后调用将使用该变量的上一个值
