...
```mermaid
graph LR;
    P[/源文件/]-->A(预处理器cpp)-->B(宏展开, 处理条件编译)--> D(处理#include)-->I[/.i 预处理文件 /]
```
...
```mermaid
graph LR;
    A[/.i 文件/] --> B(词法分析) --> C(语法分析) --> D(语义分析) --> E(中间代码生成) --> F(代码优化) --> G(目标代码生成) --> H[/.s 汇编文件/]
```
...
编译器逐字符扫描源代码, 根据词法规则将字符流转换为有意义的记号(Token)。 例如, int a = 10; 会被分解为：关键字 int、标识符 a、运算符 =、整数常量 10、分隔符 ;。
...
- 不允许有多个强符号：如果多个目标文件定义了同名的强符号(如两个文件都定义了 int g_value = 10;), 链接器会报 multiple definition 错误。
...
> 如果 file1.c 中 int a;, file2.c 中 int a = 10;, 链接不会报错, 最终 a 的值为 10。但这种行为极易引发 Bug, 建议在 C 语言中使用 extern 声明, 或在 C++ 中严格避免未初始化的全局变量。
...
```mermaid
graph TB
    subgraph 虚拟地址空间 高地址到低地址
        direction LR
        
        K["内核空间 (Kernel Space) <br/> 0xFFFFFFFFFFFFFFFF"]
        
        subgraph 用户空间
            direction LR
            Stack["栈 (Stack) <br/> 局部变量、函数参数、返回地址 <br/> 向下生长 ⬇️"]
            
            Space1[" ...."]
            
            MMap["内存映射段 (mmap) <br/> 动态库、共享内存、文件映射"]
            
            Heap["堆 (Heap) <br/> 动态分配的内存 (malloc/new) <br/> 向上生长 ⬆️"]
            
            BSS["BSS 段 (.bss) <br/> 未初始化的全局/静态变量"]
            Data["数据段 (.data) <br/> 已初始化的全局/静态变量"]
            Text["代码段 (.text) & 只读数据 (.rodata) <br/> 机器指令、常量"]
        end
        
        U["用户空间底部 (0x0000000000000000)"]
    end

    style K fill:#f9d0c4,stroke:#333,stroke-width:2px
    style Stack fill:#d4edda,stroke:#28a745
    style Heap fill:#fff3cd,stroke:#ffc107
    style BSS fill:#d1ecf1,stroke:#17a2b8
    style Data fill:#d1ecf1,stroke:#17a2b8
    style Text fill:#e2e3e5,stroke:#383d41
```

```mermaid
flowchart TB
    %% 定义左侧 ELF 文件结构
    subgraph ELF_Structure ["ELF 文件结构 (左侧)"]
        direction TB
        
        ELF_H["ELF header (文件头)"]
        
        subgraph Gray1 [" "]
            direction TB
            PHT["program header table (程序头表)"]
        end
        ...
        %% 连接左侧节点
        ELF_H --- PHT --- Init --- Text --- LibcF --- Fini --- Rodata --- LibcS --- LibcA --- Tdata --- Ctors --- Dtors --- Data --- Comment --- Debug --- Line --- Strtab --- Symtab --- Other
    end

    %% 定义右侧 内存布局
    subgraph Memory_Layout ["进程内存布局 (右侧 - 32位经典布局)"]
        direction TB
        ...
        Heap["堆 (向上增长)"]
        Global["全局数据区<br/>(全局变量、静态变量)"]
        Const["常量区<br/>(一般常量、字符串常量)"]
        Code["程序代码区"]
        AddrB["0x08048000"]
        ...
        %% 连接右侧节点
        AddrTop --- Kernel --- AddrK --- Stack --- Unmapped1 --- DynLib --- AddrM --- Unmapped2 --- Heap --- Global --- Const --- Code --- AddrB --- Res --- AddrZero
    end
    ...
    %% 样式调整
    style Gray1 fill:#e0e0e0,stroke:#none
    style Gray2 fill:#e0e0e0,stroke:#none
    style Gray3 fill:#e0e0e0,stroke:#none
    style Gap1 fill:#e0e0e0,stroke:#none
    style Gap2 fill:#e0e0e0,stroke:#none
    style Reserve fill:#e0e0e0,stroke:#none
    
    style Text fill:#fff,stroke:#333
    style Rodata fill:#fff,stroke:#333
    style Data fill:#fff,stroke:#333
    style Code fill:#fff,stroke:#333
    style Const fill:#fff,stroke:#333
...
存放全局/静态变量。.data 包含已初始化的数据, .bss 包含未初始化的数据(OS 在加载时自动将其清零)

#### stack

由编译器自动管理, 用于函数调用

存放局部变量、寄存器上下文等。从高地址向低地址生长。速度快, 但空间有限(默认通常为 8MB), 滥用会导致栈溢出 (Stack Overflow)。
