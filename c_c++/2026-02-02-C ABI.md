...
`API`(application programming interface)是人与人、源码与源码之间的契约
...
`ABI`(application binary interface) 是机器与机器、模块与模块之间的契约, 决定了编译器、链接器、加载器和操作系统如何执行你的代码

受众是编译器后端、链接器、OS内核和CPU

当源代码被编译成 0101 的机器码后, API 就消失了, 取而代之的是 ABI
...
(2) 数据布局 (data layout)：结构体的内存对齐规则(Padding)、大小端序(Endianness)
...
(1) 目标文件格式：ABI 定义了 .o / .obj 文件的结构(如 ELF、PE/COFF、Mach-O)

(2)符号解析：链接器根据 ABI 规定的符号表格式, 将多个目标文件“缝合”在一起

如果两个库的 C++ ABI 不同(比如 GCC 和 MSVC 编译的), 链接器会因为找不到匹配的 Mangling 符号而报 Undefined Reference 错误
...
如果修改了结构体的内部字段顺序, 或者改变了虚函数的声明顺序, 源码依然能完美编译通过(因为 API 没变)

但在运行时, 旧的二进制程序会去错误的内存偏移量读取数据, 或者跳转到错误的虚函数地址, 直接导致 Segmentation Fault (段错误) 或数据损坏

解决方式：必须将所有依赖该库的下游代码全部重新编译(这在大型项目中是灾难性的)
...
❌ 改 `enum` 值含义	            | 虽然二进制没变, 但逻辑语义变了, 属于隐式 ABI 破坏
❌ 增删虚函数 (c++)	            | 导致 `vtable` 布局改变, 虚函数调用会跳转到错误的地址(直接 Segfault)
...
最经典的 C ABI 封装模式(类似 c 语言中的 pimpl 模式)
...
当我们需要传递复杂配置时, 直接传递结构体容易因为后续增加字段而破坏 ABI
...
`MSVC` 和 `gcc` 的虚函数表(`vtable`)布局、异常处理机制、名字修饰完全不同

甚至同一个编译器的不同版本(如 `gcc 4` 和 `gcc 5`) `ABI` 都不兼容

- `extern "C"` 解决了什么?

解决`name mangling`问题, 让 C++ 函数拥有 C 风格的符号名
...
C 语言没有 new/delete, 更重要的是, 跨模块的内存分配必须遵循"谁分配, 谁释放"原则, 否则会导致堆损坏

- C++ 异常怎么处理?

C 语言没有异常机制(只有 setjmp/longjmp)
