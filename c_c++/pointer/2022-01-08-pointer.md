...
而指针变量是对应类型指针的变量, 与`int`变量 `char`变量一样
...
当指针变量存储了某个普通变量的地址时, 就说指针指向该变量

指针变量本身也占用内存空间, 有自己的变量名和地址, 但指针变量的值是目标变量的地址
