...
`cmake` 是一个跨平台`c/c++`构建系统生成工具, 通过解析`CMakeLists.txt`自动生成适用于不同平台的构建文件(如 `Makefile`、`Ninja` 文件、`Visual Studio` 工程文件等)

它通过配置、生成、构建三个过程将项目源文件、依赖库和编译器选项统一管理, 使构建过程标准化、跨平台化
...
`CMakeLists.txt`是`cmake`配置文件, 用于定义项目构建过程

`cmake`根据其中指令生成构建系统(如`Makefile`等), 随后通过构建工具进行编译和链接, 生成可执行文件或库
...
`cmake`读取解析`CMakeLists.txt`, 检查系统环境、依赖库、编译器等设置, 生成对应平台构建文件
...
此时所有中间文件都生成到`build/`下

- 使用其他目录构建文件

使用`source/CMakeList.txt`, 并在`build/`下生成构建文件
...
将构建产物按`CMakeLists.txt`中设置安装到指定位置
...
若`CMakeLists.txt`中未指定安装根路径, 可通过`--prefix`手动指定

```cmake
install(TARGETS main RUNTIME DESTINATION bin)
```

设构建目录`build/`, 以`test/`为根路径安装

```sh
cmake --install build --prefix test
```

可执行文件会安装到`test/bin/`下
...
`cmake`编译生成目标文件所运行操作系统名称, 交叉编译时可用来指定目标平台类型
...
构建会生成`build/main`与`build/src/hello/hello` 两个可执行文件
...
- 示例, 将`hello_1/`、`hello_2/`下源文件生成可执行文件
...
生成`libtest_api.a`与`libtest_api.so`
...
通过划分子目录与主目录`CMakeLists.txt`, 实现构建文件生成树结构

例如通过多`CMakeLists.txt`, 实现生成库并链接库生成可执行文件
...
- 示例, 子目录`test_api/`中生成动态库,链接到`src/`下可执行文件中
...
`cmake 3.11`及以上版本引入`FetchContent`模块, 可直接下载第三方库编译
...
可通过`.cmake`文件实现三方库下载过程, 并在主`CMakeLists.txt`中调用
...
修改根目录`CMakeLists.txt`, 将`fmt`库安装逻辑解耦
...
通过交叉编译器可在`AMD64`平台编译生成`ARM`架构文件
...
示例, 使用`aarch64-linux-gnu-gcc`与`aarch64-linux-gnu-g++`
