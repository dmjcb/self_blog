...
构建后生成libtest_api.a与libtest_api.so
...
可通过划分子目录与主目录CMakeLists.txt, 实现构建文件生成树结构
...
- 示例, 子目录test_api/中生成动态库并链接到src/下可执行文件中
...
构建后生成build/main可执行文件与build/src/test_api/libtest_api.so库文件
