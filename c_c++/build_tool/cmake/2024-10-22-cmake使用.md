...
`CMake` 是一个跨平台c/c++构建系统生成工具, 通过解析`CMakeLists.txt`自动生成适用于不同平台的构建文件(如 Makefile、Ninja 文件、Visual Studio 工程文件等)
...
修改根目录`CMakeLists.txt`, 将fmt库安装逻辑解耦
