...
它通常由标准库实现, 底层一般封装了 malloc

当类内部重载了 operator new 时, 可通过 ::operator new 绕过类版本, 调用系统默认分配器
...
而 make_shared 会将对象内存和控制块内存合并为一次分配, 不仅减少了分配开销, 还提高了缓存命中率, 并保证了异常安全性

### c++17 内存对齐支持 (Over-aligned Types)

在 c++17 之前, 如果类型的对齐要求超过了默认对齐(如 SIMD 指令需要的 32 字节或 64 字节对齐), 标准的 operator new 无法保证正确对齐

c++17 引入了带对齐参数的 operator new 重载: 
