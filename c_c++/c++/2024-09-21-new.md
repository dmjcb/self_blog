...
`operator new`是可重载函数, 负责从自由存储区(堆)中分配内存(类似于`malloc`函数), 不调用构造函数
...
操作符从`free store`(自由存储区)上为对象动态分配内存空间

`free store`是 c++ 基于 `new`的抽象概念, 可以是堆还可以是静态存储区, 取决于 `operator new` 在哪里为对象分配内存, 凡通过 `new` 所申请内存即为自由存储区
...
内存分配成功则是返回 `void` 指针, 需要通过强制类型转换成指定类型
