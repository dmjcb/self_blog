...
c++ 是一门支持多种编程范式的语言, 其中面向对象编程(OOP) 是其最核心的特性之一

面向对象通过封装、继承和多态三大基石, 极大地提高代码的复用性、可维护性和扩展性

## encapsulation(封装)

封装是指将数据(属性)和操作数据的函数(方法)捆绑成一个独立的单元(即类), 并隐藏对象的内部实现细节, 仅对外暴露必要的接口

封装通过访问控制修饰符(public、protected、private)来限制外部对类成员的访问, 从而保护对象状态的完整性, 提高代码的安全性

### class(类)

类(class)是对象的抽象模板, 对象(object)是类的具体实例
...
`object`是根据`class`定义所创建的具体实体, 具有自己状态(由类数据成员存储)和行为(由类成员函数定义)
...
int main() {
    std::cout << Base::s_value << std::endl;

    Base obj;
    obj.s_value++;

    std::cout << obj.s_value << std::endl;

    Base *p = new Base;
    std::cout << p->s_value << std::endl;
...
int main() {
    ...
    std::cout << "p1 m_value = " << p1.m_value << std::endl;
    std::cout << "p2 m_value = " << p2.m_value << std::endl;
...
public:
    void show() {
        std::cout << m_base_value << std::endl;
    }
};
...
int main() {
    ...
    obj.show();
    std::cout << obj.m_base_value << std::endl;
...
```sh
error: 'int Base::m_base_value' is protected within this context
```
...
属于类, 没有 this 指针, 只能访问静态成员

可通过类名直接调用(Base::func())

- 非静态成员函数

属于对象, 隐含一个 this 指针(指向当前对象), 可以访问类的所有成员
...
private:
    std::string m_name;
public:
    // this 指针隐含在参数中: void setName(Person* const this, std::string name)
    void setName(const std::string& name) {
        this->m_name = name; // 使用 this 区分成员变量与参数
    }
};
```
...
(3) 继承中调用, 在继承关系中, 如果基类有默认构造函数, 那么派生类在构造时会自动调用基类默认构造函数(除非派生类构造函数显式地调用基类其他构造函数)

##### 析构函数

在对象销毁时自动调用, 用于执行清理操作, 如释放动态分配内存

其不接受任何参数, 不能被重载, 也不能被显式调用
...
```sh
this pointer = 0x7ffeee3c6690
p address = 0x7ffeee3c6690
```
...
```sh
error: 'int Base::m_private' is private within this context
```
...
```sh
error: 'int Base::m_private' is private within this context
error: 'int Base::m_public' is inaccessible within this context
```
...
Derived class show function (hidden)
```
...
```sh
Base constructor called
SubA constructor called
Show SubA
SubA destructor called
Base destructor called
Base constructor called
SubB constructor called
Show Base
SubB destructor called
Base destructor called
```
...
class Base {
public:
    virtual void do_func_1() {
        std::cout << "Base do_func_1" << std::endl;
    }

    virtual void do_func_2() {
        std::cout << "Base do_func_2" << std::endl;
    }
}
...
public:
    ...
    void do_func_1() override {
        std::cout << "Sub do_func_1" << std::endl;
    }
}
```
...
```mermaid
graph LR
    subgraph "Base::do_func_2"
        B(address: 0x401F10)
    end
    subgraph "Base::do_func_1"
        A(address: 0x401ED0)
    end
...
public:
    virtual void do_show() {
        std::cout << "Base do_show function" << std::endl;
    }
};
...
public:
    void do_show() override {
        std::cout << "Sub do_show function" << std::endl;
    }
};
...
```mermaid
graph TD;
    X(指针p调用函数 do_show)-->A(指针p为Sub类类型)-->B(根据vptr找到Sub类vtable)-->C(检索vtable获取函数地址)-->D(Sub类override基类虚函数 do_show, 调用Sub类实现)
```
...
class Complex {
   public:
        ...
        Complex(double read, double imag) : m_real(read), m_img(imag) {}

        friend istream & operator >> (istream &, Complex &) {
            std:: >> c.m_real >> c.m_img;
        }

        friend ostream & operator << (ostream &, Complex &) {
            std:: << c.m_real << " + " << c.m_img << "i" <<std::endl;
        }

        Complex operator + (Complex &) {
            return Complex(this->m_real + c.m_real, this->m_img + c.m_img);
        }
