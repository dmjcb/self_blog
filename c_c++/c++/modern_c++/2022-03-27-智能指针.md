...
`std::unique_ptr`是一种独占所有权的智能指针

特点是同时只能有一个`std::unique_ptr`指向对象, 无法被复制, 可通过`std::move`转移所有权

使用 RAII(Resource Acquisition Is Initialization)原理管理动态内存, 超出作用域时自动调用 delete 释放其管理的内存
...
`reset()`方法可以重新分配新的对象，或者释放当前对象的所有权，使 `std::unique_ptr` 变为空
...
`std::shared_ptr` 是一种共享所有权的智能指针, 多个`std::shared_ptr`可指向同一个对象
...
使用 `std::make_shared` 工厂函数来创建和初始化`std::shared_ptr`
...
可以通过解引用操作符 `*` 来访问 `std::shared_ptr` 所管理的对象
...
使用 `use_count()` 方法获取当前 `std::shared_ptr`引用计数
...
```sh
p value: 0x7fff2b92f7c8, *p = 255
*sp value: 255
gp value: 0x5700ec19c6d0, *gp = 255
```

## std::weak_ptr

`std::weak_ptr`弱引用智能指针, 不控制对象生命周期, 因此不会增加对象的引用计数

用于打破`shared_ptr`循环引用, 避免无法释放内存
...
`std::weak_ptr` 必须和 `std::shared_ptr` 配合使用
...
`expired()` 方法用于检查所观察 `std::shared_ptr` 是否已经失效(即所管理对象是否已经被销毁)
