...
在 **C 语言**中, 定义结构体变量时通常需要带上 `struct` 关键字, 为了简化书写, 常结合 `typedef` 使用: 
...
结构体的大小(`sizeof`)通常**不等于**其成员大小之和, 为了提高 CPU 访问内存的效率, 编译器会进行**内存对齐**
