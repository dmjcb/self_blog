...
动态库(`dynamic library`)是一种在程序运行时被加载到内存中的库文件, 包含可以被多个程序同时所使用代码和数据

动态库在windows与类unix操作系统中分别以动态库链接库(`dynamic link library`, 简称DLL)与共享对象文件(`shared object file`)两种方式实现

### 对比

动态链接库和共享对象文件概念上相似, 都是用于实现代码和数据共享, 允许程序在运行时加载, 而不是在编译时静态地链接到库

两者分别属于不同操作系统平台, 其具体实现和文件格式有所不同, 但在概念上等价

#### 动态链接库

动态链接库(DLL)是windows操作系统中动态库实现方式, 库文件以`.dll`作为扩展名, 加载由windows加载器(loader)负责, 在程序启动时或根据需要动态地加载DLL

当程序加载一个DLL时, 并不会将DLL整个内容复制到自己地址空间中, 而是仅在需要时引用DLL中函数或数据

#### 共享对象文件

共享对象文件是类unix操作系统(如linux、macOS等)中动态库实现方式

共享对象文件以`.so`作为扩展名, 加载由动态链接器(dynamic linker)负责, 会在程序运行时解析和加载所需共享对象文件

### 特点

#### 运行时加载(runtime loading)

程序需要使用动态库中函数时, 编译阶段只会生成对函数引用信息, 不会将函数实现包含在程序中

链接阶段, 链接器会将引用信息解析为动态库中符号, 不会将动态库内容复制到程序中

程序执行到需要使用动态库中函数时, 操作系统会根据程序中引用信息和动态库路径信息, 将动态库加载到内存中
...
编译阶段编译器会将动态库符号和导入函数信息写入所生成中可执行文件特定区段
...
调用时发现报错, 使用`ldd `查看可执行文件依赖, 发现libtest_api.so库未找到, 可通过三种方法解决
...
生成库时, 因C语言没有`name mangling`机制, 生成函数符号名仍然为`AddNum`
...
![](/assets/image/20241227_222657.jpg)
...
- 示例, 通过类调用libclass_api.so

修改class_api.hpp, 生成动态库
...
C语言若想调用C++动态库, 则库文件需要在导出函数名前添加`extern "C"` 或用 `extern "C" {}`包裹, 否则无法调用
