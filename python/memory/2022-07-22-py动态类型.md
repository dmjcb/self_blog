...
python缓存整数和短字符串, 因此每个对象在内存中只存有一份, 引用所指对象相同,即使使用赋值语句.也只是创造新引用, 而不是对象本身

python没有缓存长字符串、列表及其他对象, 可以由多个相同对象,可以使用赋值语句创建出新对象
...
使用某个引用作为参数传递给getrefcount()时, 参数实际上创建一个临时引用, 因此结果会比期望多1
...
比如某个新建对象, 被分配给某个引用, 对象引用计数变为1

若引用被删除, 对象引用计数为0.那么该对象就可以被垃圾回收