...
在项目`settings.py`里`INSTALLED_APPS`列表中添加应用名, 使程序找到该服务
...
主目录下 settings.py文件中`DATABASES`为默认配置, 会建立 sqlite3 数据库
...
| `DateField`     | 日期类型, `python` `datetime.date()`实例, 例如 2020-08-05                                                   |
...
| `primary_key`  | 若为字段设置了 `primary_key=True`, 则当前字段变为主键; 隐含 `null=False` 和 `unique=True`  |
...
与`settings.py`同级目录下建立`database_router.py`文件
...
在 app 目录下新建 `templates/`

注册模板目录, 新建模板目录需在项目设置文件中进行注册

打开主目录 `settings.py`, 找到 `TEMPLATES`, 将新增模板路径添加至`'DIRS'`项中:
...
运行, 访问`http://127.0.0.1:8000/app/index`
...
访问 `http://127.0.0.1:8000/app/db`, 在网页上显示数据库中所有元素信息

在 `app/templates` 下新建 `db.html`
...
运行项目, 访问 `http://127.0.0.1:8000/app/get_db`
...
此时可匹配`127.0.0.1:8080/index/<字符>`, 并且<字符>部分值将作为 index 函数第二个参数
...
前面 `w+` 传给 `v1`, 后面 `w+` 传给 `v2`, 此时可匹配`127.0.0.1:8000/value/<字符1>/<字符2>`这类 `url`, 并且字符1将传给v1参数, 字符2将传给v2参数
...
若想将`polls`的`detail`视图的URL更换为`polls/specifics/12/`, 那么仅仅只需要在`polls/urls.py`中, 将对应的正则表达式改成下面这样的就行, 所有模板中对它的引用都会自动修改成新的链接
