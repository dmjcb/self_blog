...
`sorted`函数将按列表升序进行排序, 返回新list对象, 原list保持不变

| 参数       | 含义                                                         |
| ---------- | ----------------------------------------------------------- |
| `iterable` | 可迭代对象                                                   |
| `cmp`      | 比较函数, 返回值大于则返回 `1`, 小于则返回 `-1`, 等于则返回 `0` |
...
`sort` 方法仅操作列表; `sorted` 方法返回新`list`
...
`len`是内建函数, `sorted`函数在排序时会用`len`去获取每个字符串长度来排序

#### 复杂排序

如由元组构成的列表, 若要按照元组中第二个元素排序, 可用`lambda`定义匿名函数
...
`sorted`也可以根据多个字段来排序, 例如要先根据age排序, 若age相同则根据grade排序
...
需要使用`functools`模块中`cmp_to_key`来指定比较函数是什么
...
| 函数                     | 描述                                      |
| ------------------------ | ---------------------------------------- |
| `get(key, default=None)` | 返回指定键值, 若值不在字典中返回`default`值 |
| `has_key(key)`           | 若键在字典里返回`true`, 否则返回`false`    |
...
| 函数            | 描述                         |
| --------------- | --------------------------- |
| `update(dict2)` | 把字典dict2键值对更新到dict里 |
...
| 函数       | 作用                                |
| --------- | ----------------------------------- |
| `loads()` | 将`json`转化成`dict`                 |
| `dumps()` | 将`dict`转化成`json`                 |
| `load()`  | 读取`json` 转成`dict`                |
| `dump()`  | 将`dict`转化成`json`后写入`jsonw`文件 |
...
在字符串`[start, end]`范围里查找`子串s`, 若存在返回首个索引, 否则返回`-1`
...
`sys` 即 `system`, 该模块提供接口用于访问解释器自身使用和维护的变量, 同时模块中还提供函数可与解释器进行比较深度交互
...
小端存储时, 数据低位也存储在存储空间低位地址中, 此时`sys.byteorder`值为`"little"`
...
另一种存储顺序是"大端存储", 即数据高位字节存储在存储空间低位地址上, 此时`sys.byteorder`值为`"big"`

### executable

该属性是字符串, 在正常情况下值是当前运行解释器对应可执行程序所在绝对路径
...
`collections` 是内置模块, 提供很多方便且高性能关于集合操作
...
`namedtuple()` 相当于直接定义了一个新类, 其比直接定义 `class` 方式省很多空间, 同时其返回值是一个 `tuple`
