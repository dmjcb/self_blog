...
SSH(Secure Shell)是一种**加密远程登录协议**, 常用于服务器运维、远程命令执行、文件传输及端口转发
...
手动复制需先将本地`id_rsa.pub` 内容追加到远程主机的 `.ssh/authorized_keys`里

也可使用`ssh-copy-id`, 若是windows系统, 需先于powershell中执行,
