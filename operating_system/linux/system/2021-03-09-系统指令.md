...
默认情况(非重定向)时, 会输出nohup.out到当前目录, 若不可写, 则输出重定向到`${HOME}/nohup.out`
...
`write`把缓冲区buf前`n bytes`写入到与文件描述符`fd`关联文件中, 返回实际写入字节数
...
从文件描述符`fd`相关联文件里读入`n bytes`数据, 并放到数据区buf中
...
`FILE`结构是间接地操作系统文件控制块(FCB)来实现对文件操作
