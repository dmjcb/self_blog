...
nvidia/cuda:12.1.0-cudnn8-runtime-ubuntu22.04   f3a7fb39fa3f       5.48GB         2.11GB
pt2zh-fastapi:latest                            34e98feee7fd       19.1GB         6.81GB
pt2zh-web:latest                                a838f715a105       19.1GB         6.81GB
video-translator:latest                         42ebd63b6cb3       21.1GB         8.38GB
```
...
- 示例, 将镜像`python:3.8-slim`保存为file_python3_8_slim文件

```sh
docker save python:3.8-slim -o file_python3_8_slim
```
...
- 示例, 解压file_python3_8_slim

```sh
docker load -i file_python3_8_slim
```
...
25109184c71b   9 days ago   CMD ["/bin/sh"]                                  0B        buildkit.dockerfile.v0
<missing>      9 days ago   ADD alpine-minirootfs-3.23.3-x86_64.tar.gz /…   9.11MB    buildkit.dockerfile.v0
```
...
多阶段构建允许在单`Dockerfile`中使用多`from`, 并可用 `as` 为构建阶段命名
...
- 示例, 把 file1.txt 和 file2.txt 添加到`/app`下
...
- 示例, 把 archive.tar.gz 文件添加到镜像中, 并自动解压到`/app`下
...
重置之前为当前系统架构配置的`qemu`用户模式二进制文件

- `-p yes`

通知`qemu-user-static`探测当前系统架构, 并自动配置适当`qemu`用户模式二进制文件, `yes`确认操作
