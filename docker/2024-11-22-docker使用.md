...
为加速docker镜像拉取, 可编辑`/etc/docker/daemon.json`, `registry-mirrors`可替换为其他可用仓库路径
...
docker镜像可以包含网络设置信息, 如IP地址和端口映射等, 在容器启动时会被`docker`引擎读取并应用到容器上, 从而实现容器与外部网络通信
...
入口点默认参数, 指定`entrypoint`后, CMD中参数会被传递给`entrypoint`命令作为执行参数
